...
def base64_to_hex(base64_input: str) -> str:
    """
    Convert a base64 string to a hexadecimal string
    """
    # Add padding if necessary
    missing_padding = len(base64_input) % 4
    if missing_padding:
        base64_input += '=' * (4 - missing_padding)
...
def repeating_XOR_encode(plaintext, key):
    """
    Encode a plaintext with repeating-key XOR, 
    ...
    candidate_key = key * (int(len(plaintext)/len(key))+1)
    candidate_key = candidate_key[:len(plaintext)]
    output_byte = XOR(plaintext,candidate_key)
    return output_byte

#TODO: score functie hier telt enkel letters, maar nog niet letterfrequentie (bijv: hoe vaak 'e'?)
...
def break_vigenere(ciphertext: bytes) -> bytes:
    ...
    Parameters:
        ...
        num_guesses (int): number of guesses you want to output
...
#input
with open('set1_challenge6.txt', 'r', encoding='utf-8') as file:
    input_str = file.read().strip() #base64
input_bytes = base64.b64decode(input_str) #bytes

#output
plaintext_b, key = break_vigenere(input_bytes)
plaintext = plaintext_b.decode('ascii', errors='replace')
with open('set1_challenge6_solution.txt', 'w', encoding='utf-8') as file:
